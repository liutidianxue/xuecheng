...
public class PageService {
    ...
    public QueryResponseResult findList(int page, int size, QueryPageRequset queryPageRequset){
        ...
        //设置条件值(站点id)
        if (StringUtils.isNotBlank(queryPageRequset.getSiteId())) {
            cmsPage.setSiteId(queryPageRequset.getSiteId());
        }
        //设置模板id作为查询条件
        if (StringUtils.isNotBlank(queryPageRequset.getTemplated())) {
            cmsPage.setTemplateId(queryPageRequset.getTemplated());
        }
        //设置页面别名作为查询
        if (StringUtils.isNotBlank(queryPageRequset.getPageAliase())) {
        ...
        Page<CmsPage> all = cmsPageRepository.findAll(example,pageable);//实现自定义条件查询
