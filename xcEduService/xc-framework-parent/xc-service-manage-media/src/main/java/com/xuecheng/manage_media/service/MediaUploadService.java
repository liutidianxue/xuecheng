...
public class MediaUploadService {
     ...
     * @return 文件路径
     */
    public ResponseResult register(String fileMd5, String fileName, Long fileSize, String fileExt) {
        //1 检查文件在磁盘上是否存在
        //文件所属目录的路径
    ...
    public ResponseResult mergechunks(String fileMd5, String fileName, Long fileSize, String mimetype, String fileExt) {
        ...
        //直接用数据的排序，和lamdba表达式简化
        //创建一个合并文件
        String filePath = this.getFilePath(fileMd5, fileExt);
        File mergeFile = new File(filePath);
        //执行合并
        mergeFile = this.mergeFile(files, mergeFile);
        if (mergeFile == null) {
            //合并文件失败
    ...
    private File mergeFile(File[] files, File mergeFile) {
        try {
            //如果合并文件已经存在则删除，否则创建新文件
            if(mergeFile.exists()){
                mergeFile.delete();
            }else{
                //创建一个新文件
